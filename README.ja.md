...
Agent Work Snapshot は、そのための最小の報告フォーマットです。各エージェントや担当者は、会話全文や作業ログ全部ではなく、タスクに紐づいた短いsnapshotを出します。管理者はそれを見て、WBSやissue trackerなどの正本と照合し、必要に応じてレビューし、承認します。

ポイントは、作業する側がタスクの正本を勝手に書き換えないことです。snapshotはあくまで報告です。検証、承認、WBSやcanonical task modelへの反映は、別の仕組みや人間の判断で行います。これにより、AIが増えても、人間の管理者がプロジェクト全体を見失いにくくなります。
